...
def get_related_words(word):
    ...
    if len(tokens) > 1:
        ...
        # Aggiungi sinonimi
        for syn in wn.synsets(tokens[0], pos=wn.NOUN, lang='ita'):
...
def correction(word):
    ...
    if (word.upper() == 'NOT') or (word.upper() == '(NOT'):
        return word
    else:
...
def execute_query(original_query, classifier, sentiment_analysis, correct_spelling, synonym):
    ...
    ''''RIGA DI CODICE DA CAMBIARE MANUALMENTE DIPENDENTEMENTE SE SI UTILIZZA LA NDCG O LA GUI:'''
...
if __name__ == "__main__":
    ...
    results = execute_query(query, sentiment_classifier, V3, True, True)
